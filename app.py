...
def get_file_headers(uploaded_file):
    """Helper to safely extract headers from uploaded CSV or Excel files."""
    try:
        ...
        st.error(f"Error reading headers from {uploaded_file.name}: {e}")
        return []

def read_full_file(uploaded_file):
    """Helper to read the entire file into memory."""
...
with col1:
    ...
    if tracker_file:
        tracker_headers = get_file_headers(tracker_file)
        tracker_pim = st.selectbox("Map PIM ID Column", [""] + tracker_headers, key="t_pim")
        tracker_rrp = st.selectbox("Map RRP Column", [""] + tracker_headers, key="t_rrp")
        tracker_md = st.selectbox("Map Markdown (MD) Column", [""] + tracker_headers, key="t_md")
    ...
    if sku_file:
        sku_headers = get_file_headers(sku_file)
...
with col2:
    ...
    if "Shopee" in mode or "Both" in mode:
        ...
        if shopee_file:
            shopee_headers = get_file_headers(shopee_file)
    ...
    if "Lazada" in mode or "Both" in mode:
        ...
        if lazada_file:
            lazada_headers = get_file_headers(lazada_file)
...
if st.button("🚀 Run Automation Process", type="primary", use_container_width=True):
    ...
    if not error_found:
        with st.spinner("Processing files and calculating pricing logic..."):
            try:
                # Load Full Data
                df_tracker = read_full_file(tracker_file)
                df_sku = read_full_file(sku_file)
                ...
                with pd.ExcelWriter(output_buffer, engine='openpyxl') as writer:
                    ...
                    if shopee_file and ("Shopee" in mode or "Both" in mode):
                        df_shopee = read_full_file(shopee_file)
                        mismatch_rows = []
                        upload_rows = []
                    ...
                    if lazada_file and ("Lazada" in mode or "Both" in mode):
                        df_lazada = read_full_file(lazada_file)
