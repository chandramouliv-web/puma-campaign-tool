...
def calculate_campaign_price(rrp, srp, voucher_pct):
    ...
    campaign_price = round(
        base_price * (1 - voucher_pct / 100),
        2
    )
...
def generate_lazada_campaign(
    ...
    zecom["RRP"] = pd.to_numeric(
        zecom[rrp_col],
        errors="coerce"
    ).fillna(0)

    zecom["SRP"] = pd.to_numeric(
        zecom[srp_col],
        errors="coerce"
    ).fillna(0)

    zecom["Campaign Price"] = zecom.apply(
        lambda x: calculate_campaign_price(
            x["RRP"],
            x["SRP"],
            voucher_pct
        ),
    ...
    # Fill Campaign Price Column

    if "Campaign Price_x" in result.columns:

        result["Campaign Price_x"] = (
            result["Campaign Price_y"]
        )
...
st.set_page_config(
    page_title="Lazada Campaign Generator",
    layout="wide"
)

st.title("🏷️ Lazada Campaign Price Generator")
...
if (
    ...
    if st.button(
        ...
        result = generate_lazada_campaign(
            zecom_df,
            content_df,
            lazada_df,
            article_col,
        ...
        output = export_excel(result)

        st.download_button(
            label="⬇ Download Campaign File",
            data=output,
            file_name=f"Lazada_Campaign_{voucher_pct}pct.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
